...
test("banco de preguntas mantiene estructura y coherencia por bloque", () => {
  ...
  for (const q of allQuestions) {
    ...
    if ("optionExplanations" in q) {
      assert.equal(Array.isArray(q.optionExplanations), true, `optionExplanations debe ser array en ${q.id}`);
      assert.equal(q.optionExplanations.length, q.options.length, `optionExplanations incompleto en ${q.id}`);
      for (const [idx, text] of q.optionExplanations.entries()) {
        assert.ok(typeof text === "string" && text.trim().length > 0, `optionExplanations[${idx}] vacío en ${q.id}`);
      }
    }
  }
});
