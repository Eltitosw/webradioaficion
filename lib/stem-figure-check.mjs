...
export const STEM_FIGURE_PATH_RE = /^images\/quiz\/[A-Za-z0-9._-]+\.(svg|png|webp|jpg|jpeg)$/i;
...
export function checkStemFigures(all, webRoot, fail) {
  for (const q of all) {
    ...
    if (!STEM_FIGURE_PATH_RE.test(rel)) {
      fail(
        `Pregunta ${id}: stemFigure no cumple el patrón permitido (images/quiz/nombre.svg|png|webp|jpg|jpeg): ${JSON.stringify(rel)}`,
      );
      continue;
    }
