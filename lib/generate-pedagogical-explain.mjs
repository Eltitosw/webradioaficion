...
function explainElectricidad(stem, correct) {
  ...
  if (/realimentaci[oó]n|feedback|reinyect|salida.*entrada|entrada.*salida/.test(s)) {
    return `Tomar parte de la salida y reinyectarla a la entrada es realimentación (feedback): puede estabilizar o modificar la ganancia. No confundir con rectificación ni demodulación. «${c}».`;
  }
  if (/transferencia m[aá]xima de potencia|m[aá]xima.*potencia.*entre/i.test(s)) {
    return `La máxima transferencia de potencia entre dos circuitos se produce cuando las impedancias están adaptadas (carga y fuente acopladas según el temario). No confundir con ondas estacionarias ni con «antena despejada». «${c}».`;
  }
  if (/pilas|bater[ií]as/.test(s) && /serie/.test(s)) {
    if (/intensidad/i.test(c)) {
      return `Con pilas idénticas en serie circula la misma intensidad en toda la rama (un solo camino); la tensión total es la suma de las de cada pila. «${c}».`;
    }
    if (/tensi[oó]n/i.test(c)) {
      return `En serie las tensiones de cada pila se suman; la intensidad es la misma en todo el circuito serie. «${c}».`;
    }
  }
  ...
  if (/resistencias en serie|resistencias en paralelo|equivalente/.test(s) && !/pilas|bater[ií]as/.test(s)) {
