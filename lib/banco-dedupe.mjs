...
import { areParaphraseDuplicates } from "./question-paraphrase.mjs";
import { getRecencyMeta, recencyScore } from "./question-recency.mjs";

/** @param {object} q */
function hasCertifiedRasterFigure(q) {
  const path = q?.stemFigure;
  return typeof path === "string" && path.includes("-original.") && fs.existsSync(path);
}

/** @param {string} id */
function sourcePriority(id) {
  if (id.startsWith("ofic-")) return 0;
  if (id.startsWith("ure-")) return 1;
  if (id.startsWith("fedi-")) return 2;
...
export function pickDuplicateWinner(candidates, cribadoIds) {
  ...
  return [...candidates].sort((a, b) => {
    ...
    const aF = hasCertifiedRasterFigure(a) ? 1 : 0;
    const bF = hasCertifiedRasterFigure(b) ? 1 : 0;
...
/** @param {object} a @param {object} b @param {Set<string>} [cribadoIds] */
function compareWinnerPriority(a, b, cribadoIds) {
  const winner = pickDuplicateWinner([a, b], cribadoIds);
  return winner.id === a.id ? -1 : 1;
}
 ...
 * @param {Set<string>} cribadoIds
 */
export function dedupeBankByParaphrase(bankById, cribadoIds) {
  const items = [...bankById.values()].sort((a, b) => compareWinnerPriority(a, b, cribadoIds));
  /** @type {object[]} */
  const kept = [];
  const removed = [];

  for (const q of items) {
    if (kept.some((k) => areParaphraseDuplicates(k, q))) {
      removed.push(q.id);
      continue;
    }
    kept.push(q);
  ...
  return {
    bankById: new Map(kept.map((q) => [q.id, q])),
    removed,
    paraphraseGroups: removed.length,
  };
}
...
export function dedupeBankByStem(bankById, cribadoIds) {
  ...
  const para = dedupeBankByParaphrase(next, cribadoIds);

  return {
    bankById: para.bankById,
    removed: [...removed, ...para.removed],
    duplicateGroups: [...groups.values()].filter((g) => g.length > 1).length,
    paraphraseRemoved: para.removed.length,
    paraphraseGroups: para.paraphraseGroups,
  };
}
