...
/** FEDI bloque d: «CODIGO "Q"» / «CODIGO 'Q'» (comillas entre codigo y Q). */
export const CODIGO_Q_STEM_RE =
  /(?:abreviatura.*)?c[oó]digo\s*["']?\s*q|del\s+c[oó]digo\s*["']?\s*q|c[oó]digo\s*["']?\s*q\s+corresponde/i;

/** Código Q, socorro operativo, abreviaturas de tráfico. */
export const PART2_QCODE_STEM_RE = new RegExp(
  [
    String.raw`\bq(?:sy|rm|sb|rt|ra|th|sl|rv)\b`,
    CODIGO_Q_STEM_RE.source,
    String.raw`qu[eé]\s+abreviatura\s+indica`,
    String.raw`jerga.*qsy`,
    String.raw`señal\s+.*\bsocorro\b`,
    String.raw`señal radiotelegr[aá]fica de socorro`,
    String.raw`señal radiotelef[oó]nica de socorro`,
    String.raw`radiotelegr\w{0,12}fica\s+de\s+socorro`,
    String.raw`llamada de socorro`,
    String.raw`mensaje de socorro`,
    String.raw`señal internacional de seguridad`,
    String.raw`pan\s*pan\b`,
    String.raw`buena pr[aá]ctica.*finalizar una comunicaci`,
    String.raw`abreviatura\s+ttt`,
    String.raw`abreviatura\s+xxx`,
    String.raw`abreviatura.*\bcl\b`,
  ].join("|"),
  "i",
);
...
const PART2_OPERACION_STEM_RE = new RegExp(
  [
    String.raw`mayday`,
    String.raw`securit[eé]`,
    String.raw`\bc[oó]digo q\b`,
    CODIGO_Q_STEM_RE.source,
    String.raw`\brst\b`,
    String.raw`fon[eé]tic`,
    String.raw`deletreo`,
    String.raw`socorro inminente`,
    String.raw`urgencia radi`,
    String.raw`señal de socorro`,
    String.raw`señal de urgencia`,
    String.raw`señal de seguridad`,
    String.raw`señal\s+.*\bsocorro\b`,
    String.raw`señal radiotelegr[aá]fica de socorro`,
    String.raw`radiotelegr\w{0,12}fica\s+de\s+socorro`,
    String.raw`señal internacional de seguridad`,
...
export function topicIdPart1(stem) {
  ...
  if (
    /antena|dipolo|radial|propagaci|ionosfera|troposfera|coaxial|yagi|balun|guia de onda|diagrama.*radiaci|reflexiones lunares/i.test(
      s,
    ) &&
  ...
  if (
    /receptor|transmis|transceptor|mezclad|modul|demodul|oscilador|portadora|squelch|selectividad|sensibilidad|superheterodin|intermodulaci|ganancia de radiofrecuencia|control autom[aá]tico de ganancia|vox-man|load knob|control de carga|emisor de|amplificador|detecci[oó]n sirve|send.?rec|agc\b|s-meter|medidor.?s\b|mando vox|conmutador.*emis|modo de emisi[oó]n|fon[ií]a.*banda lateral|paso final|alta tensi[oó]n de placa|excitador|sintoniz|usb.*lsb|modo de recepci[oó]n.*usb|interferencias son m[aá]s frecuentes/i.test(
      s,
    )
  ) {
  ...
  if (
    /onda|polarizaci|frecuencia|hf\b|vhf|uhf|ancho de banda|espectro|dbm|dbuv|clase de emisi|abreviatura (lf|mf|hf|vhf|uhf)|\b(lf|mf|hf|vhf|uhf)\b indica|nomenclatura de bandas|gama de frecuenc|comprendida entre \d+ y|ondas centim[eé]tricas|banda lateral|\bssb\b|fading|desvanecimiento|s[ií]mbolo (lf|mf|hf)|decibelio|modo de emisi[oó]n.*\bcw\b/i.test(
      s,
    )
  ) {
