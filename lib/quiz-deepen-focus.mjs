...
/** Unidades y magnitudes que no aportan tema: evitan emparejar por «MHz», «dB», etc. */
...
/** Viñetas que describen un dibujo/figura: no encajan si la pregunta no la trae. */
const FIGURE_REF_RE = /\b(dibujo|figura|gr[aá]fic|esquema|diagrama|pantalla|recuadro|cuadro)\b/i;
...
function tokens(text) {
  return String(text || "")
    ...
    ?.filter((t) => !STOP.has(t) && !UNIT_TOKENS.has(t) && !/^\d+$/.test(t)) ?? [];
}
...
export function pickDeepenFocusLines(q, opts = {}) {
  ...
  const ranked = candidates
    .filter((line) => hasFigure || !FIGURE_REF_RE.test(line))
