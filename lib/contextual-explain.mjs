...
export function inferExplainTopic(stem, topicId) {
  ...
  if (
    /provincia|distrito|cifra.*identifica|indicativo|distintivo|prefijo\s*e[eabc]?\b|sufijo\s*pan|empadron|licencia cept|harec|cept\b|prefijo de pa[ií]s|menci[oó]n\s*\/p|\/mm\b|prefijo\s+eg\b|distintivo temporal|reasignar el distintivo/i.test(
      s,
    )
  ) {
    return "licencias-indicativos";
  }
  if (/señal internacional de seguridad|señal de alarma en radiotelefon|señal de urgencia en radiotelefon/i.test(s)) {
  ...
  if (
    /radioaficionado|aficionad|servicio de radioaficion|licencia de estaci|autorizaci[oó]n de radio|tr[aá]fico entre estaciones|distintivo de llamada|repetidor|desatendid|harec|cept\b|diploma de operador/i.test(
      s,
    )
  ) {
    if (/indicativo|distintivo|prefijo|sufijo|distrito|cifra.*identifica|\/p\b|\/mm\b/i.test(s)) {
      return "licencias-indicativos";
    }
    if (/antena|terraza|comunidad|instalador|sistema radiante|tejado|soportes/i.test(s)) {
...
export function synthesizeReason(stem, correct, topicId) {
  ...
  if (/siguiente|figura|esquema|diagrama|gr[aá]fica|pantalla/i.test(s)) {
    if (/distintivo|indicativo|ea\d|eb\d|provincia|localidad/i.test(s)) {
      return `El indicativo español combina prefijo E, cifra de distrito y sufijo; la cifra identifica la provincia o territorio. «${c}».`;
    }
  ...
  if (/indicativo de especializaci[oó]n|especializaci[oó]n/.test(s) && !/especializaci[oó]n remota|monitorizaci/i.test(s)) {
    return `El indicativo de especialización distingue titulaciones, roles o funciones del operador (no es el distintivo ordinario EA… de la estación). Se asocia a habilidades o funciones acreditadas, no a emergencias ni a ser solo numérico. «${c}».`;
  }
  if (/indicativo fijo/.test(s)) {
    return `Un indicativo fijo es el distintivo estable de una estación (no cambia con cada QSO como un móvil/portátil en otro distrito). La respuesta correcta es «${c}».`;
  }
  if (/indicativo variable/.test(s)) {
    return `Un indicativo variable puede cambiar según el servicio o modalidad (p. ej. móvil/portátil frente a estación fija). «${c}».`;
  }
  ...
  if (/señal de urgencia|pan[\s-]?pan/i.test(s)) {
    return `Pan-Pan (tres veces) es la señal de urgencia sin peligro grave inmediato; Mayday reserva el socorro. «${c}».`;
  }
  ...
  if (/frecuencia de resonancia.*lc|resonancia.*circuito\s*lc|f0\s*=\s*1\/\(2/i.test(s)) {
    return `La resonancia en LC ocurre cuando las reactancias de L y C se equilibran; f0 = 1/(2π√(LC)) en el modelo ideal. «${c}».`;
  }
  ...
  if (/capacidad de un condensador|condensador se mide en/i.test(s)) {
    return `La capacidad almacena carga eléctrica; en el SI se mide en faradios (F) y submúltiplos (µF, nF, pF). «${c}».`;
  }
  ...
  const stemBrief = stem.replace(/\s+/g, " ").trim().slice(0, 72);
  return `En este enunciado («${stemBrief}${stem.length > 72 ? "…" : ""}»), el criterio de examen apunta a «${c}». Repasa ${topicHint} en el temario y fija qué regla o magnitud distingue la opción correcta de los distractores.`;
}
